...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const data = await catalogService.getCatalog(baseUrl, { force });
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.query.withEpisodes === "true" || req.query.withEpisodes === "1") {
      response.data.results = await catalogService.filterItemsWithEpisodes(response.data?.results || []);
